...
@Service
public class ProdRepertoryServiceImpl implements ProdRepertoryService{
		@Autowired
		...
		@Override
		public PageObject<ProdRepertory> findPageObjects
		(String name, Integer pageCurrent) {
			...
			int rowCount = 0;
			try {
				rowCount = prodRepertoryDao.rowCount(name);
			} catch (Exception e) {
				e.printStackTrace();
			...
			List<ProdRepertory> records = prodRepertoryDao.doFindPageObject(name, startIndex, pageSize);

			//5.对查询的结果进行封装
			PageObject<ProdRepertory> pageObject = new PageObject<>();//jdk1.7以后才允许
...
//			if(StringUtils.isEmpty(ids))
			...
			try{//数据库是专门的服务器,tomcat也是一台专门的服务器
				rows=prodRepertoryDao.deleteObjects(ids);
		...
		@Override
		public int saveObject(ProdRepertory entity) {
			//1.参数合法验证
			if(entity==null)
			...
			int rows;
			try{//运维监控
				rows=prodRepertoryDao.insertObject(entity);
			}catch(Throwable t){
				t.printStackTrace();
				//报警....
		...
		@Override
		public int updateObject(ProdRepertory entity) {
			//1.合法性验证
			  if(entity==null)
			  ...
			  throw new IllegalArgumentException("商品单价不能为空");
			  //2.将数据更新到数据库
			  int rows=prodRepertoryDao.updateObject(entity);
			  //3.对结果进行验证
			  if(rows==0)
		...
		@Override
		public ProdRepertory findObjectById(Integer id) {
			return prodRepertoryDao.findObjectById(id);
		}
		...
		public int updateObjectById(Integer id) {
			...
			ProdRepertory repertory = prodRepertoryDao.findObjectById(id);
			Double pnum = repertory.getPnum();
			if(pnum==0){
				throw new ServiceException("库存不足");
			}else{
				repertory.setPnum(pnum--);
			}
			int row = prodRepertoryDao.updateObject(repertory);
			System.out.println(repertory);
			return row;
		}

		@Override
		public String updateStatusById(Integer id, Integer newStatus) {
			if(id==null||id==0)
				throw new IllegalArgumentException("id不合法");
			int row = prodRepertoryDao.updateStatusById(id, newStatus);
			if(row==0)
				throw new ServiceException("修改状态失败");
			String result = newStatus==1?"上架成功":"下架成功";
			return result;
		}
